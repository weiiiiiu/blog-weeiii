...
- 站点 logo 与 favicon 改用 AI 生成的 Labubu 头像图（粉底、头部撑满画面）。此前用个人照片，在 36px 顶栏与 16px 标签栏下细节尽失。保留粉色背景而非抠成透明，因米色绒毛在浅色主题的白色顶栏/标签栏上对比度过低。顶栏由圆形遮罩 `rounded-full` 改为圆角 `rounded-lg`，避免圆形裁切削掉兔耳。`favicon.ico` 输出 16/32/48/64 多尺寸。
