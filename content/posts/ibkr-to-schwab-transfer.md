---
title: 从 IBKR 转持仓到嘉信（Schwab）：ACATS 全流程
...
| 方式 | 用途 | 用不用得上 |
| --- | --- | --- |
| **ACATS**<br>Automated Customer Account Transfer Service | 两家**美国券商之间**转移美股 / ETF / 期权 / 美债 / 现金，全自动 | ✅ 标准路径 |
| **FOP**<br>Free of Payment | 转移**非美证券**到美国以外的机构（加拿大、欧洲、亚太） | ❌ 嘉信主要托管美国证券，用不到 |
...
- ⚠️ **期权**：到期日在 **7 个工作日以内的期权通常不予转移**，别在临近到期时发起
...
股票本身通过 **ACATS** 转移；成本价（买入价、买入日期、各税务批次 lots）通过 DTCC 的 **CBRS**（Cost Basis Reporting Service）单独传输，**不在 ACATS 报文里**。所以典型现象是：**股票先到嘉信，成本价几天后才补上。**

- **法定时限**：对 **covered securities**（一般指 2011 年及以后买入），IBKR 必须在转移完成后 **15 天内**把成本价发给嘉信。这是 IRS 强制要求
- **Non-covered / 2011 年之前买入**的持仓，成本价**可能不会自动转**，需要自己保留记录
- 到账后请在嘉信**逐笔核对数量、买入价、买入日期、批次**；有缺失或错误，可要求嘉信向 IBKR 索取 ACATS / CBRS 文件
- 成本价到账前**如果卖出**，报税（Form 8949）时要自己提供成本价

> **ACATS 转移本身不是卖出、不触发资本利得税**，持仓和持有期保持连续。这是它相对"卖掉→转现金→重买"的核心价值。

---

## 七、几个必踩的坑

1. **先确认嘉信能托管你所有的标的**——尤其是非美 ADR、期权、碎股（fractional shares）、某些基金。不可转资产会被拒绝或残留在 IBKR
2. **避开期权到期周**，以及有未结算交易、未到账分红的时点
3. **转之前把 IBKR 的历史对账单全部下载存档**。成本价补不齐时（尤其老仓位）这是唯一凭证
4. 本文是流程性信息整理，**非税务或投资建议**。费率和规则会变，提交前以两家官方客服的最新确认为准
...
| 问题 | 答案 |
| --- | --- |
| **方式** | **ACATS**；FOP 是给非美证券用的，用不到 |
| **IBKR 收费** | **$0**，全部 / 部分都免费，也无关闭费 |
| **嘉信收费** | 转入 **$0** |
| **谁发起** | **嘉信端**（接收方录入），无需在 IBKR 操作 |
| **需要材料** | IBKR 账户号 / 名称 / 类型、SSN、IBKR 最新月结单、（partial）标的清单 |
| **前提** | 两边注册信息一致；账户无欠款 / 纠纷 |
| **耗时** | **5–7 个工作日**（SEC 上限 6 天，复杂情况可达约 3 周） |
| **转移中能否交易** | **冻结，不能**；partial 只冻结转出标的；临近到期期权不转 |
| **转完账户** | 变空但**不自动关闭**、**无闲置费**；主动关闭则 3 个月收尾期 |
| **成本价** | **会转**，经 CBRS 单独传，covered 证券法定 15 天内；到账后核对 |
